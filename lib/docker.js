...
function dockerDownloadAndImportLayer(opts, callback) {
    ...
    function addImageFile(arg, next) {
        ...
        /*
         * Don't let the on('data',...) handler above un-pause the stream here,
         * or else the first chunk or two of the file might get dropped, since
         * storeFileFromStream will probably go back to the event loop (creating
         * directories) before calling .pipe()
         */
        stream.pause();
